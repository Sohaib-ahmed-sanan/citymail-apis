...
if ($has_key == 200) {
    ...
    if ($valid_key == 200) {
        try {
            ...
            $query = "SELECT shipments.*, delivery_status.name AS status_name, cities.city As destination_city,customers.business_name,customers.acno
            FROM  `shipments`
            ...
            WHERE shipments.company_id = '$company_id' AND shipments.is_deleted = 'N' AND shipments.payment_status = '1' AND shipments.status IN (14,16) $more
            ";
            // echo $query;
            // die;
            $dbobjx->query($query);
